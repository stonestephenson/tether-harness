...
Verifier-integrity guard (anti-tamper):
  ...
  reports ONCE (exit 2 + the verifier diff on stderr — a block where the tool can
  block, a user-visible report where it can't) and then re-baselines so it never
  nags twice for the same change. A RED run just gets a tamper note on the normal
  red report (no re-baseline: reverting to the accepted verifier goes green
  silently).

Safety:
  * `stop_hook_active` guard: never block twice in a row (no infinite loop).
  * time-boxed; on timeout it fails OPEN (lets the agent stop) silently — a slow
    check must not earn an error report on every finish.
...
# so it works no matter where the hooks are installed. Your wiring should pass a
# stable per-session `session_id` in the stdin JSON (see WIRING.md) so baselines
# from different sessions never collide.
STATE_DIR = os.path.join(tempfile.gettempdir(), "tether-done-gate-state")
