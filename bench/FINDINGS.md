...
| 3 | Weekly cap sizes vs measured gated-arm token burn — do ~610 runs fit in ~2–3 weeks? | (needs the burn multiplier from a pilot cell) | **partial (probe data)** | Two trivial-task probes give a floor: ≈180 output tok/turn; P1-real 4 turns / $0.11-equiv, P1-raw 18 turns / $0.31-equiv (subscription — dollars are phantom). Cache-read tokens scale ~linearly with turns and dominate totals. Real-task burn (the actual number) still needs a pilot cell; the per-run turn cap bounds worst-case. |
