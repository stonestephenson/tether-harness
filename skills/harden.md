...
- AGENTS.md (project or your tool's global config) has accumulated
...
- **AGENTS.md** (project first, then your tool's global copy): imperative
...
|---|---|---|---|
| **a** | existing linter's config (ruff / clippy / eslint rule) | the rule maps to a lint code the project's linter already knows (e.g. a banned import → ruff `flake8-tidy-imports` banned-api) | every edit (verify-on-edit) + every finish |
| **b** | check appended to the project verifier (**`.tether/verify.sh`**, or `.codex`/`.claude` if that's what the project uses) | expressible as a fast text/path predicate | every finish (done-gate) |
| **c** | your tool's permission/policy config (Claude Code permission deny rules, Codex `config.toml` approval policy, opencode `permission` rules, …) | "never run/touch X" — a tool-call shape the tool's config can express | at the tool call itself |
| **d** | tiny pre-tool-call guard (a PreToolUse hook, permission callback, or plugin — whatever your tool's hook system offers) | the check needs to inspect tool *input* at call time | at the tool call itself |
...
- **Expect the tamper flag.** Appending to the verifier mid-session trips the
  done-gate's verifier-integrity guard at the next finish — that's the guard
