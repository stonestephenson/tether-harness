...
## Coupling — handoff's cold audit runs this playbook
The handoff playbook's Agent A opens its audit by running catchup for real (invoking
it, or reading this playbook and following it), and holds its orientation to a
contract: what/state, build/run/test commands, conventions, backlog (handoff Step 2).
If you change Step 1's pointer-following behavior or the shape of Step 4's report,
re-check handoff Step 2 — a silent mismatch there degrades the audit.
