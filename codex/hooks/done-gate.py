#!/usr/bin/env python3
"""
done-gate hook (Stop) — Tier-1 verification, at task completion. Codex edition.
...
Opt-in per project (so it never runs a heavy suite by surprise). It runs iff:
  ...
  * a `.tether/verify.sh`, `.codex/verify.sh`, or `.claude/verify.sh` exists (cwd).
...
Verifier-integrity guard (anti-tamper):
  ...
  SpecBench; the original live observation was a Codex agent rewriting
  .codex/verify.sh). The first invocation that resolves a verifier records its
  SHA-256 in per-session state; later invocations re-hash. On a change it always
  tells the user (systemMessage), and when the run is green it blocks ONCE with
  the diff so the agent surfaces the change for confirmation or reverts it — then
  re-baselines so it can never loop. A red run just gets a tamper note on the
  normal red block (no re-baseline: reverting to the accepted verifier goes green
  silently).
...
# so it works no matter where the hooks are installed (not tied to ~/.codex).
STATE_DIR = os.path.join(tempfile.gettempdir(), "codex-done-gate-state")
