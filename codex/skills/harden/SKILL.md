---
name: harden
description: Compile accumulated corrections ("never use X", "always run Y first") from prose — AGENTS.md rules, nits the user has repeated — into MECHANICAL enforcement at the cheapest sufficient tier (existing linter config → a .codex/verify.sh check → a Codex config policy → last-resort PreToolUse guard). Use when a correction gets repeated, when feedback has accumulated, at a hardening milestone, or when the user says "harden this", "make that stick", "enforce this rule", "stop letting me/you do X", or "compile my corrections". Always proposes before writing; judgment-only preferences stay prose.
---
...
- AGENTS.md (project or `~/.codex/AGENTS.md`) has accumulated "never/always/don't"
  lines.
...
- **AGENTS.md** (project first, then the global `~/.codex/AGENTS.md`): imperative
...
| **b** | check appended to the project verifier (**`.codex/verify.sh`**, or `.tether/verify.sh` if that's what the project uses) | expressible as a fast text/path predicate | every stop (done-gate) |
| **c** | Codex config policy (`config.toml` — approval/sandbox rules) | "never run/touch X" — a tool-call shape the config can express | at the tool call itself |
...
(banned-api); this project doesn't → tier (b), appended to `.codex/verify.sh`:
