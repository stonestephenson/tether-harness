...
Spawn **two cold `codex exec` runs IN PARALLEL** (launch A in the background, then
B) — fresh sessions with zero context by design: Agent A as
`codex exec --sandbox workspace-write` (it may build/test) and Agent B as
`codex exec --sandbox read-only`, each run from the repo directory. Substitute the
...
> 1. **Orient the way a real cold agent does: invoke the `catchup` skill** (it is
>    installed in this environment; if skill invocation isn't available in this
>    run, read `~/.codex/skills/catchup/SKILL.md` and follow its steps as your
...
>    (iv) the backlog / next tasks. **Fallback:** if neither the skill nor its
>    SKILL.md is available, or the orientation is missing any of those four, then
>    read the entry docs directly (README, CLAUDE.md/AGENTS.md, ROADMAP/CHANGELOG,
...
`codex exec` isn't available in this environment (no auth, restricted sandbox), fall
back to doing both audits yourself under an explicit "pretend you have never seen
this repo; trust only what the files say" framing — weaker, but better than nothing;
tell the user you fell back. (The sibling degradation — the exec child can't invoke
the catchup skill — is already handled inside Agent A's prompt: it reads the
installed SKILL.md, else the entry-doc list, and reports which.)
