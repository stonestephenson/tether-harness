...
- **What:** when I try to end a turn, it runs the project's fast check command
  (`.codex/verify.sh` — also honors `.tether`/`.claude` and `$VERIFY_CMD`) and, if
  it's red, **blocks the finish** and hands the failures back. Loop-guarded: it
  blocks **once per stop cycle** (`stop_hook_active`), so an immediately repeated
  stop passes even if still red — the gate prods with the failure list; it is built
  never to trap the agent in a block loop.
...
- **Integrity:** "green means green" only if the checker itself didn't quietly change —
  ...
  **Known limits (by design):** the baseline is taken at the session's *first* stop —
  ...
  `VERIFY_CMD`/`CLAUDE_VERIFY_CMD` string) is hashed — scripts or tests that the
  verifier *calls* are outside the guard.
...
- **What:** spawns fresh cold subagents (`codex exec` runs) to prove a zero-context
  agent could build, run, test, and extend the project from the docs alone — then
  fixes whatever they trip on. Its onboarding auditor orients by running `catchup`
  for real, so the audit tests the composed docs×catchup path an actual cold agent
  takes (the two skills are coupled — each SKILL.md carries the contract/change-site
  note).
