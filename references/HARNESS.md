...
- **What:** when a compaction is about to run and the git working tree is dirty
  (staged, unstaged, or untracked), it emits the un-externalized state — the file list
  plus a "preserve this work in the summary" instruction on stdout (for tools that can
  inject context into the compaction prompt) and a user-facing warning on stderr,
  pointing at ship / handoff / context-health.
- **Why:** compaction is lossy; the summary won't preserve uncommitted work unless
  ...
  one moment it can be checked deterministically.
- **When:** wire it to your tool's pre-compaction event (see `WIRING.md`). This edition
  is advisory and never blocks; tools whose pre-compact event *can* block (Claude Code)
  should use `main`'s blocking edition. Fails open: no git, not a repo, any error → the
  compaction proceeds untouched.
...
- **What:** runs the project's full quality gates, gets the diff reviewed in **fresh
  context** (one cold read-only pass of your tool given only the diff + intent;
...
- **What:** gathers accumulated prose corrections (AGENTS.md "never/always" lines, nits
  ...
  sufficient tier: existing linter config → a `.tether/verify.sh` check → your tool's
  permission/policy config → (last resort) a pre-tool-call guard. Always proposes before
  writing; every compiled rule carries a provenance note (origin + date) so it stays
