...
- **What:** when I try to end a turn, it runs the project's fast check command
  (`.claude/verify.sh`; on opencode `.tether/verify.sh`) and, if it's red, **blocks
  the finish** and hands the failures back — once per stop cycle (the loop guard
...
- **Integrity:** "green means green" only if the checker itself didn't quietly change —
  ...
  **Known limits (by design):** the baseline is taken at the session's *first* idle —
...
- **What:** spawns fresh cold subagents (`opencode run` children) to prove a
  zero-context agent could build, run, test, and extend the project from the docs
  alone — then fixes whatever they trip on. Its onboarding auditor orients by
  executing the installed `catchup` command for real, so the audit tests the composed
  docs×catchup path an actual cold agent takes (the two commands are coupled — each
