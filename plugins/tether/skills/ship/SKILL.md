...
2. **Fresh-context review of the judgment questions.** Prefer the built-in
   `/code-review` skill when the environment has it. Otherwise spawn **one** cold
   read-only subagent given ONLY the diff and a one-line statement of intent
   ("this change is supposed to X"), reporting back correctness risks, simpler
   alternatives, and anything that contradicts the stated intent. One reviewer, no
   personas — the value is the fresh context, not a role label. If neither path
   exists (no /code-review, no subagents), fall back to careful in-thread review
   and say so — the review happened, but it wasn't cold.
