#!/usr/bin/env bash
# Regression test for the opencode edition of verify-on-edit.py, done-gate.py,
# and pre-compact-guard.py.
# Run:  bash opencode/tests/verify-hooks.test.sh   (from the repo root)
#
# The opencode contract (see opencode/plugins/tether-verify.js): the JS plugin
# translates opencode events into Claude-Code-shaped JSON on stdin, and a hook
# signals "problem" with a NON-ZERO EXIT + text on stderr — there is no
# decision:block JSON channel and no hard-block. pre-compact-guard is the
# inverted PreCompact: stdout is INJECTED into the compaction prompt, stderr
# goes to the user, and nothing can block the compaction. These tests drive
# the Python hooks with the exact payload shapes the plugin sends.
...
DG_STATE="$FIX/opencode-done-gate-state"
