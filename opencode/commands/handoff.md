...
Spawn **two cold `opencode run` children IN PARALLEL** (launch A in the background,
then B) — fresh sessions with zero context by design: Agent A as a plain
`opencode run` from the repo directory (it may build/test), Agent B as
`opencode run --agent plan` (read-only). Leave `--pure` OFF — the auditors should
experience the repo the way a real session does, plugins included. Substitute the
absolute repo path for `<REPO>`. Use these prompts close to verbatim — they are
tuned to catch the full range of handoff gaps:
...
> 1. **Orient the way a real cold agent does: execute the `/catchup` command** —
>    read `~/.config/opencode/commands/catchup.md` (the installed command — always
>    the current version, never a paraphrase) and follow its steps as your
>    orientation procedure. Take the orientation it produces — you are the "user"
>    it reports to; don't wait for input, continue with this audit. The
>    orientation must deliver: (i) what the project is and its current state,
>    (ii) the build/run/test commands, (iii) the conventions / definition of done,
>    (iv) the backlog / next tasks. **Fallback:** if that command file doesn't
>    exist here, or the orientation is missing any of those four, then read the
>    entry docs directly (README, CLAUDE.md/AGENTS.md, ROADMAP/CHANGELOG, docs/),
>    skim the source tree — and REPORT that you fell back and exactly which
>    deliverables were missing: that is itself a finding, not a detour.
...
catchup-free control, so the verdict never rests solely on the catchup path). If
`opencode run` children can't be spawned in this environment, fall back to doing
both audits yourself under an explicit "pretend you have never seen this repo;
trust only what the files say" framing — weaker, but better than nothing; tell the
user you fell back. (The sibling degradation — the child can't reach the installed
catchup command — is already handled inside Agent A's prompt: it falls back to the
entry-doc list and reports it.)
