...
2. **Fresh-context review of the judgment questions.** Spawn **one** cold
   read-only opencode run given ONLY the diff and a one-line statement of intent
   (the `plan` agent is read-only; `--pure` keeps plugins out of the reviewer):
   `git diff HEAD > /tmp/ship-review.diff && opencode run --pure --agent plan
   -f /tmp/ship-review.diff "Review this diff. It is supposed to <intent>.
   Report correctness risks, simpler alternatives, and anything that contradicts
   the stated intent."` One reviewer, no personas — the value is the fresh
   context, not a role label. If a cold `opencode run` isn't possible in the
   environment, fall back to careful in-thread review and say so — the review
