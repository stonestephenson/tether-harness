...
echo "      done-gate on session.idle (with the verifier-integrity tamper guard);"
echo "      pre-compact-guard on experimental.session.compacting (injects dirty-tree"
echo "      state into the compaction prompt — opencode can't block a compaction)."
